            ...
            if prob.status not in ["infeasible", "unbounded"]:
                ...
                if res < bestres_r - 1e-3:  # even the relaxed problem sucks. forget about this branch then
                    ...
                    if res > bestres_r: # if a valid solution then this is the new best
                        ...
                        bestres_r = res
                        node.res_r = res
                        node.bool_vars_r = [node.bool_vars[i].value for i in range(c)]
...
def subproblem(v, w, theta, t, ite,type):
    ...
    if type==0:
        #part 2: bnb for existing solution
    ...
    if type==1:
        #part 3: bnb for optimal solution
...
def ADP():
    ...
    for j in range(c):
        ...
        for t in range(T):
            ...
            '''
            if j !=c-1:
                if a2[a2_sorted[j]]==a2[a2_sorted[j+1]]:
                    mp.add_constraint(vp[t, a2_sorted[j]] == vp[t, a2_sorted[j + 1]])
                else:
                    mp.add_constraint(vp[t, a2_sorted[j]] >= vp[t, a2_sorted[j+1]])
            '''
        '''
    ...
    # mp.add the constraint when t=0?
    v = np.full((T + 1, c), 0.0)
    theta = np.zeros(T + 1)
    w = np.zeros(T + 1)
    pi = np.full(T, np.inf)
    ...
    while sum(pi) > sig * Z:
        ...
        for t in range(1, T + 1):
            p1, p2, p3, x, y, pi[t - 1] = subproblem(v, w, theta, t, ite,0)
            print('pi:', pi[t - 1],x, y)
            r10=r1(p1, p2, p3)
    ...
    while sum(pi) > sig * Z:
        ...
        for t in range(1, T + 1):
            sub_t_start = time.time()
            p1, p2, p3, x, y, pi[t - 1] = subproblem(v, w, theta, t, ite,1)
            print('pi:', pi[t - 1],x, y)
            r10 = r1(p1, p2, p3)
            r20 = [r2(j, p1, p2, p3) for j in range(c)]
            r30 = [r3(j, p1, p2, p3) for j in range(c)]
...
#sys.stdout = open(r'C:\Users\siqhe\PycharmProjects\pythonProject2\BB\part_3_BB_log.' + str(c) + 'choice'+str(choice)+'txt', 'w')
print(c)
T = c * 2
